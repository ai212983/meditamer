...
pub(crate) fn record_wifi_connect_success() {
    ...
    // Association is not a DHCP lease. Drop any prior lease until the Wi-Fi
    // task publishes the current stack address.
...
pub(crate) fn set_wifi_ipv4(ip: Option<[u8; 4]>) {
    let raw_ip = ip.map(u32::from_be_bytes).unwrap_or(0);
    WIFI_IPV4.store(raw_ip, Ordering::Relaxed);
}
