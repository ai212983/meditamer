...
pub(crate) const UI_SETTINGS_CAPACITY: usize = CATALOGUE_CAPACITY;
pub(crate) const UI_SETTINGS_WRITE_DEBOUNCE_MS: u64 = 1_500;
pub(crate) const UI_SETTINGS_MIN_WRITE_INTERVAL_MS: u64 = 5_000;
pub(crate) const UI_SETTINGS_RETRY_BACKOFF_MS: u64 = 30_000;

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct PersistedUiSettings {
    pub(crate) ambient_binding: Option<EntryId>,
    pub(crate) pins: Vec<EntryId, UI_SETTINGS_CAPACITY>,
    pub(crate) enabled_overlays: Vec<EntryId, UI_SETTINGS_CAPACITY>,
    pub(crate) startup_entry: Option<EntryId>,
    pub(crate) startup_overlays: Vec<EntryId, UI_SETTINGS_CAPACITY>,
    pub(crate) enablement_configured: bool,
    pub(crate) startup_overlays_configured: bool,
}
...
impl UiSettings {
    pub(crate) fn resolve<const CAPACITY: usize>(
    ...
    pub(crate) fn persisted(&self) -> PersistedUiSettings {
        self.persisted.clone()
    }

    pub(crate) fn ambient_binding(&self) -> EntryId {
        self.persisted
            .ambient_binding
            .expect("settings resolution always supplies the base ambient fallback")
    }

    pub(crate) fn pins(&self) -> &[EntryId] {
        self.persisted.pins.as_slice()
    }

    pub(crate) fn overlay_enabled(&self, id: EntryId) -> bool {
        self.persisted.enabled_overlays.contains(&id)
    }

    pub(crate) fn startup_entry(&self) -> Option<EntryId> {
        self.persisted.startup_entry
    }

    pub(crate) fn startup_overlay_enabled(&self, id: EntryId) -> bool {
        self.persisted.startup_overlays.contains(&id)
    }

    pub(crate) fn select_ambient(&mut self, id: EntryId) -> bool {
    ...
    pub(crate) fn toggle_overlay(&mut self, id: EntryId) -> bool {
...
pub(crate) struct UiSettingsPersistence {
    current: UiSettings,
...
impl UiSettingsPersistence {
    pub(crate) fn new(current: UiSettings) -> Self {
    ...
    pub(crate) fn current(&self) -> &UiSettings {
        &self.current
    }

    pub(crate) fn select_ambient(&mut self, id: EntryId, now_ms: u64) -> bool {
    ...
    pub(crate) fn toggle_overlay(&mut self, id: EntryId, now_ms: u64) -> Option<bool> {
    ...
    pub(crate) fn take_due(&mut self, now_ms: u64) -> Option<PersistedUiSettings> {
    ...
    pub(crate) fn complete(&mut self, success: bool, now_ms: u64) {
...
pub(crate) enum UiSettingsIntent {
...
mod tests {
    ...
    use crate::shell::catalogue::CatalogueBadge;
    use crate::shell::{
        catalogue::{CatalogueEntry, Compatibility, GlyphRef, Health, Residency, SourcePresence},
    ...
    fn ready_filter_requires_the_requested_capability() {
        ...
        assert_eq!(
            catalogue.entry(APP_ID).unwrap().badge(),
