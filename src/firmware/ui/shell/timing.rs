...
pub(crate) struct TimerServiceMetrics {
    last_handler_us: u64,
    max_gap_us: u64,
    max_runtime_us: u64,
}
...
impl TimerServiceMetrics {
    pub(crate) const fn new(now_us: u64) -> Self {
        Self {
    ...
    pub(crate) fn begin_handler(&mut self, now_us: u64) {
    ...
    pub(crate) fn finish_handler(&mut self, runtime_us: u64) {
        self.max_runtime_us = self.max_runtime_us.max(runtime_us);
    }

    pub(crate) const fn max_gap_us(&self) -> u64 {
        self.max_gap_us
    }

    pub(crate) const fn max_runtime_us(&self) -> u64 {
        self.max_runtime_us
    }
}
