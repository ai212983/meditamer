...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) struct NavigationPurge {
    pub(crate) removed_frames: usize,
    pub(crate) active_changed: bool,
}

pub(crate) struct Navigator<const CAPACITY: usize> {
    frames: Vec<NavigationFrame, CAPACITY>,
    revision: u32,
}

pub(crate) struct NavigationPlan<const CAPACITY: usize> {
...
impl<const CAPACITY: usize> Navigator<CAPACITY> {
    pub(crate) fn new(ambient: SurfaceDefinition) -> Result<Self, NavigationError> {
    ...
    pub(crate) fn prepare_open_launcher(
        &self,
    ...
    pub(crate) fn prepare_push_child(
        &self,
    ...
    pub(crate) fn prepare_back(&self) -> NavigationPlan<CAPACITY> {
        let mut next = self.frames.clone();
        if next.len() > 1 {
            next.pop();
        }
    ...
    pub(crate) fn prepare_home(&self) -> NavigationPlan<CAPACITY> {
        let mut next = self.frames.clone();
        next.truncate(1);
        self.plan(next)
    }

    pub(crate) fn owns_ambient(&self, owner: ProviderToken) -> bool {
        self.frames
            .first()
    ...
    pub(crate) fn purge_provider(&mut self, owner: ProviderToken) -> NavigationPurge {
        let Some(index) = self
            .frames
            .iter()
    ...
    pub(crate) fn frame(&self, index: usize) -> Option<NavigationFrame> {
