...
pub(crate) enum CompositionReferenceError {
...
pub(crate) struct CompositionPurge {
    pub(crate) live_overlays: usize,
    pub(crate) queued_modals: usize,
}

#[derive(Clone, Debug, Eq, PartialEq)]
pub(crate) struct CompositionDelta<const LIVE_CAPACITY: usize, const MODAL_CAPACITY: usize> {
    ...
    CompositionDelta<LIVE_CAPACITY, MODAL_CAPACITY>
{
    ...
    pub(crate) fn enter_live(&self) -> &[OverlayInstance] {
        self.enter_live.as_slice()
    }

    pub(crate) fn leave_live(&self) -> &[OverlayInstance] {
        self.leave_live.as_slice()
    }

    pub(crate) fn remove_queued(&self) -> &[OverlayInstance] {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum CompositionPlanResult {
    Admission(OverlayAdmission),
    Removal(OverlayDismissal),
    Cleanup(CompositionPurge),
}

pub(crate) struct CompositionPlan<const LIVE_CAPACITY: usize, const MODAL_CAPACITY: usize> {
...
#[derive(Clone)]
pub(crate) struct CompositionReferences<const LIVE_CAPACITY: usize, const MODAL_CAPACITY: usize> {
    ...
    CompositionReferences<LIVE_CAPACITY, MODAL_CAPACITY>
{
    ...
    pub(crate) fn plan_drop_transient(&self) -> CompositionPlan<LIVE_CAPACITY, MODAL_CAPACITY> {
        let mut next = self.clone();
    ...
    pub(crate) fn plan_provider_detach(
        &self,
        owner: ProviderToken,
    ...
    pub(crate) fn remove(&mut self, token: SurfaceInstanceToken) -> Option<OverlayDismissal> {
        let expected = self.preview_remove(token)?;
    ...
    pub(crate) fn preview_remove(&self, token: SurfaceInstanceToken) -> Option<OverlayDismissal> {
    ...
    pub(crate) fn drop_transient(&mut self) -> CompositionPurge {
        let purge = self.purge_matching(|instance| instance.lifetime == OverlayLifetime::Transient);
        let _ = self.promote_if_modal_vacant();
        purge
    }

    pub(crate) fn purge_provider(&mut self, owner: ProviderToken) -> CompositionPurge {
    ...
    pub(crate) fn live(&self, index: usize) -> Option<OverlayInstance> {
    ...
    pub(crate) fn first_transient(&self) -> Option<OverlayInstance> {
        self.live_overlays
            .iter()
