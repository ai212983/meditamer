...
pub(crate) trait SurfaceRuntime {
    type Instance;
    type EnterError;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum RollbackReason<EnterError, CommitError> {
    Entry(EnterError),
    Activation {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(crate) enum PostCommitFailure {
    OriginCleanup,
}

pub(crate) enum DestroyFailure<Instance> {
    Live(Instance),
    Audit,
}

pub(crate) enum TransitionResult<Instance, Outcome, EnterError, CommitError> {
    Committed {
        active: Instance,
        outcome: Outcome,
    },
...
pub(crate) fn execute_transition<Runtime, Outcome, CommitError>(
    runtime: &mut Runtime,
