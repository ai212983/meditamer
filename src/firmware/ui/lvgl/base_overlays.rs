...
pub(super) enum BaseOverlayKind {
    NavigationCue,
    RefreshControl,
}

impl BaseOverlayKind {
    pub(super) const fn input(self) -> OverlayInput {
        match self {
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub(super) enum OverlayEnterError {
    CallbackRoute,
    ObjectCreation,
}
...
impl ActiveOverlay {
    ...
    pub(super) fn confirm(instance: OverlayInstance) -> Result<Self, OverlayEnterError> {
        unsafe { ConfirmModal::create(instance) }.map(Self::Confirm)
    }

    pub(super) fn token(&self) -> SurfaceInstanceToken {
        self.instance().token
    }
    ...
    #[cfg(feature = "ui-provider-fixture")]
    pub(super) fn references_provider(&self, owner: ProviderToken) -> bool {
...
pub(super) struct PassiveOverlay {
    instance: OverlayInstance,
    root: *mut lv::lv_obj_t,
}

impl PassiveOverlay {
    pub(super) unsafe fn create(instance: OverlayInstance) -> Option<Self> {
...
pub(super) struct RefreshControl {
    instance: OverlayInstance,
    root: *mut lv::lv_obj_t,
