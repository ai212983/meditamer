...
use super::super::super::types::{AppEvent, SerialStatusEvent, SD_PATH_MAX, SD_WRITE_MAX};
use super::commands::{
...
fn serial_status_lines_preserve_existing_wire_format() {
    assert_eq!(
        status::format_status(SerialStatusEvent::ImuScheduler {
            sensor_odr_hz: 416,
            idle_hz: 20,
            active_hz: 104,
            active_hold_ms: 750,
    ...
    assert_eq!(
        status::format_status(SerialStatusEvent::ImuReady).as_str(),
        "imu: ready\r\n"
    );
    assert_eq!(
        status::format_status(SerialStatusEvent::ImuInitFailed).as_str(),
        "imu: init_failed; retrying\r\n"
    );
    assert_eq!(
        status::format_status(SerialStatusEvent::ImuReadError).as_str(),
