...
pub(super) fn reset_parse_wrap_diag() {
...
pub(super) fn log_parse_wrap_diag(stage: &str) {
    let count = NEXT_SLOT.load(Ordering::Relaxed).min(SLOT_COUNT);
    println!(
        "upload_http: boot_scan_only_diag parse_wrap_diag after={} count={}",
        stage, count
    );
    for idx in 0..count {
        let snap = unsafe { SNAPSHOTS[idx] };
        println!(
            "upload_http: boot_scan_only_diag parse_wrap_diag_entry after={} idx={} fn={} ret=0x{:08x} arg0=0x{:08x} arg1=0x{:08x} arg2=0x{:08x} arg3=0x{:08x}",
            stage,
            idx,
            fn_label(snap.fn_id),
            snap.ret,
        ...
        for ptr_idx in 0..PTR_ARG_COUNT {
            ...
            println!(
                "upload_http: boot_scan_only_diag parse_wrap_diag_fields after={} idx={} fn={} ptr_idx={} pre={:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x} post={:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
                stage,
                idx,
                fn_label(snap.fn_id),
                ptr_idx,
