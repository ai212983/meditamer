...
pub(super) fn reset_wdev_fiq_wrap_diag() {
...
pub(super) fn log_wdev_fiq_wrap_diag(stage: &str) {
    let count = NEXT_SLOT.load(Ordering::Relaxed).min(SLOT_COUNT);
    println!(
        "upload_http: boot_scan_only_diag wdev_fiq_wrap_diag after={} count={}",
        stage, count
    );
    for idx in 0..count {
        let snap = unsafe { core::ptr::read(SNAPSHOTS.as_ptr().add(idx)) };
        println!(
            "upload_http: boot_scan_only_diag wdev_fiq_wrap_diag_entry after={} idx={} arg0=0x{:08x} ret=0x{:08x} pre_mac_isr={} post_mac_isr={}",
            stage,
            idx,
            snap.arg0,
            snap.ret,
