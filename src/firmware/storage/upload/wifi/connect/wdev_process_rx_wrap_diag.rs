...
impl Ring {
    ...
    fn log(&self, stage: &str) {
        ...
        println!(
            "upload_http: boot_scan_only_diag wdev_process_rx_wrap_diag after={} count={}",
            stage, count
        );
        for idx in 0..count {
            let snap = unsafe { core::ptr::read(self.snapshots.as_ptr().add(idx)) };
            println!(
                "upload_http: boot_scan_only_diag wdev_process_rx_wrap_diag_entry after={} idx={} arg0=0x{:08x} arg1=0x{:08x} arg2=0x{:08x} arg3=0x{:08x} ret=0x{:08x} arg0_words={:08x}:{:08x}:{:08x}:{:08x} arg2_words={:08x}:{:08x}:{:08x}:{:08x} pre_mac_isr={} post_mac_isr={} pre_rx_sta={} post_rx_sta={}",
                stage,
                idx,
                snap.args[0],
                snap.args[1],
...
pub(super) fn reset_wdev_process_rx_wrap_diag() {
    RING.reset();
}

pub(super) fn log_wdev_process_rx_wrap_diag(stage: &str) {
    RING.log(stage);
}
