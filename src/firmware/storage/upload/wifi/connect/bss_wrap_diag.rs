...
pub(super) fn reset_bss_wrap_diag() {
...
pub(super) fn log_bss_wrap_diag(stage: &str) {
    let count = NEXT_SLOT.load(Ordering::Relaxed).min(SLOT_COUNT);
    println!(
        "upload_http: boot_scan_only_diag bss_wrap_diag after={} count={}",
        stage, count
    );
    for idx in 0..count {
        let snap = unsafe { SNAPSHOTS[idx] };
        println!(
            "upload_http: boot_scan_only_diag bss_wrap_diag_entry after={} idx={} fn={} ret=0x{:08x} arg0=0x{:08x} arg1=0x{:08x} arg2=0x{:08x} arg3=0x{:08x} ret_words={:08x}:{:08x}:{:08x}:{:08x}",
            stage,
            idx,
            fn_label(snap.fn_id),
            snap.ret,
