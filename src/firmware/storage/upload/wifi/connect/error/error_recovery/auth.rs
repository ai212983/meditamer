async fn handle_auth_reason_paths(state: &mut WifiTaskState, auth_reason: bool) -> bool {
    if !auth_reason {
        return false;
    }
    ...
    if state.auth_method_idx == 0 && state.channel_hint.is_some() {
        if let Some(next_candidate) =
            rotate_to_next_candidate(&state.ap_candidates, state.bssid_hint, &mut state.ap_candidate_idx)
        {
