...
const DIAG_WIFI: u32 = observability::LOG_DOMAIN_WIFI;
const DIAG_REASSOC: u32 = observability::LOG_DOMAIN_REASSOC;

macro_rules! diag_wifi {
    ($($arg:tt)*) => {
        if observability::log_filter_enabled(DIAG_WIFI) {
            println!($($arg)*);
        }
    };
}
...
macro_rules! diag_reassoc {
    ($($arg:tt)*) => {
        if observability::log_filter_enabled(DIAG_REASSOC) {
            println!($($arg)*);
        }
    };
}
...
pub(crate) fn net_status_snapshot() -> NetStatusSnapshot {
    let (state, ladder_step, attempt, failure_class, failure_code, uptime_ms) =
        read_status_fields();
    let observability = observability::snapshot();
    NetStatusSnapshot {
        state: state.as_str(),
        link: observability.wifi_link_connected,
        radio_quiesced: status::radio_quiesced(),
        ipv4: observability.wifi_ipv4.unwrap_or([0, 0, 0, 0]),
        listener: observability.upload_http_listening,
