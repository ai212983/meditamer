use super::*;
use crate::firmware::net::wifi::status::publish_radio_quiesced;
...
pub(super) async fn prepare_preconditions(
    ...
    while let Ok(config) = NET_CONFIG_SET_UPDATES.try_receive() {
        ...
        if let Some(updated) = config.credentials {
            if state.credentials != Some(updated) {
                ...
                observability::record_wifi_reassoc_credentials_changed();
            }
        }
        state.net_attempt = 0;
    ...
    if !service_mode::upload_enabled() {
        if !state.paused {
            ...
            observability::record_wifi_reassoc_mode_pause();
            state.paused = true;
    ...
    if state.paused {
        ...
        state.net_attempt = 0;
        state.terminal_fail_latched = false;
        observability::record_wifi_reassoc_mode_resume();
    ...
    if let Some(watchdog_started_at) = state.hard_recover_watchdog_started_at {
        ...
        if elapsed_ms >= watchdog_timeout_ms {
            observability::record_wifi_reassoc_disconnect_event(
    ...
    let active = match state.credentials {
        ...
        None => {
            ...
            .await
            {
                ...
                state.net_attempt = 0;
                state.terminal_fail_latched = false;
                observability::record_wifi_reassoc_credentials_received();
                publish_config(state.credentials, state.runtime_policy);
