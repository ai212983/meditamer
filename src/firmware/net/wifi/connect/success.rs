...
use progress::handle_connected_progress;
use recovery::recover_dhcp_no_ipv4_stall;
...
pub(super) async fn handle_connect_success(
    ...
    loop {
        ...
        if WIFI_DISCONNECTED_EVENT.swap(false, Ordering::Relaxed) {
            ...
            observability::record_wifi_reassoc_disconnect_event(disconnect_reason);
            state.dhcp_lease_reacquire_attempts = 0;
            reset_listener_timeout_guard(state);
            observability::set_wifi_link_connected(false);
            observability::set_upload_http_listener(false, None);
        ...
        if let Ok(rssi_dbm) = wifi_rssi(controller) {
            observability::record_wifi_link_rssi(rssi_dbm);
        }
