//! SPI-mode SD card probe.
//!
//! This module owns the card handle, its error and status vocabulary, and the
//! command opcodes. [`init`] brings a card up, [`io`] does single-sector
//! transfers, [`write_multi`] the CMD25 burst path, [`wait`] the busy/token
//! polling, and [`decode`] the CSD and boot-sector parsing.
...
const SD_INIT_DEADLINE_MS: u64 = 2_000;
#[allow(async_fn_in_trait)]
pub trait SdSpiBus:
    SpiBus<u8, Error = SpiError> + SetConfig<Config = SpiConfig, ConfigError = SpiConfigError>
{
    /// Keeps exclusive ownership of the SPI/DMA transaction until the driver
    /// reports completion. Racing this future against a software timer drops
    /// an in-flight ESP32 DMA transfer and leaves the peripheral unusable.
    async fn transfer_in_place_to_completion(
        &mut self,
        words: &mut [u8],
    ) -> Result<(), SdProbeError> {
        SpiBus::transfer_in_place(self, words)
            .await
            .map_err(SdProbeError::from)
    }
}
...
impl SdProbeError {
    pub fn is_timeout(&self) -> bool {
        matches!(
            self,
            Self::InitDeadlineExceeded | Self::DataTokenTimeout(_) | Self::WriteBusyTimeout { .. }
        )
    }
    ...
    SPI: SdSpiBus,
{
    ...
    /// Restores protocol state after a completed transfer or bounded protocol
    /// loop reports an error. Callers must not drop an in-flight DMA future.
    ...
    fn check_init_deadline(&mut self) -> Result<(), SdProbeError> {
        if self
            .init_deadline
            .is_some_and(|deadline| Instant::now() >= deadline)
        {
            // This check runs only between completed transfers. Raising CS
            // here leaves the protocol idle without dropping a DMA future.
            self.cs.set_high();
            self.invalidate();
            return Err(SdProbeError::InitDeadlineExceeded);
        }
        Ok(())
    }

    async fn transfer_init_bounded(&mut self, words: &mut [u8]) -> Result<(), SdProbeError> {
        self.check_init_deadline()?;
        self.spi.transfer_in_place_to_completion(words).await?;
        self.check_init_deadline()
    }
