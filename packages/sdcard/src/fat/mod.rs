//! FAT32 support.
//!
//! This module owns the on-disk domain model -- the volume, directory record,
//! and long-file-name types every stage shares. The parsing and encoding rules
//! live in the sibling modules; [`engine`] drives them as a stackless state
//! machine.
...
use directory_names::{encode_short_name, make_short_alias};
use directory_parse::{process_directory_slot, DirectoryScanState, DirectorySlotOutcome};
use list_parse::{parse_list_dir_slot, ListDirSlot};
use names_lfn::{parse_path, parse_record, path_segment_to_name, short_name_checksum};
use size_math::clusters_for_size;
use volume_parse::{
    cluster_to_lba, first_fat_partition_lba, parse_fat_size, parse_sectors_per_cluster,
    parse_total_sectors,
};
