...
| --- | --- | --- | --- |
| [`host/app_state_store_host_harness/`](host/app_state_store_host_harness/) | Firmware + Host Tooling | `scripts/host-test.sh test\|lint app-state`; aggregate host-tests/host-lint/coverage lanes | Reuses `src/firmware/app_state/*` on the host via `#[path]` shims and depends on the `shell` crate directly (ADR-0015); no production or CLI entry point exercises this path. |
...
| [`host/ui_shell_host_harness/`](host/ui_shell_host_harness/) | Firmware + Host Tooling | `scripts/host-test.sh test\|lint ui-shell`; aggregate host-tests/host-lint/coverage lanes | Re-exports the `shell` crate (ADR-0015) and hosts LVGL overlay semantics; requires `DEP_LV_CONFIG_PATH` to resolve `lightvgl-sys`'s `lv_conf.h`. |
