...
impl<const SOCK: usize> StackResources<SOCK> {
    ...
    /// Destroy the previous stack epoch so this storage can be initialized again.
    ///
    /// The exclusive `&mut self` proves that no `Stack` or `Runner` from the
    /// previous epoch remains borrowed.
    pub fn reset(&mut self) {
        ...
        unsafe {
            ...
            self.sockets.assume_init_drop();
            #[cfg(feature = "dns")]
            self.queries.assume_init_drop();
            #[cfg(feature = "dhcpv4-hostname")]
            {
                if self.hostname.option_initialized {
                    self.hostname.option.assume_init_drop();
                    self.hostname.option_initialized = false;
                }
                if self.hostname.data_initialized {
                    self.hostname.data.assume_init_drop();
                    self.hostname.data_initialized = false;
                }
            }
        }
...
impl<const SOCK: usize> Drop for StackResources<SOCK> {
    fn drop(&mut self) {
        self.reset();
    }
}
...
fn to_smoltcp_hardware_address(addr: driver::HardwareAddress) -> (HardwareAddress, Medium) {
    match addr {
        ...
        driver::HardwareAddress::Ethernet(eth) => (
            HardwareAddress::Ethernet(EthernetAddress(eth)),
            Medium::Ethernet,
        ),
        #[cfg(feature = "medium-ieee802154")]
...
impl<'d> Stack<'d> {
    ...
    fn wait<'a>(
        &'a self,
        mut predicate: impl FnMut() -> bool + 'a,
        ...
        match config {
            ConfigV4::Dhcp(c) => {
                ...
                if let Some(h) = c.hostname {
                    ...
                    hostname.data_initialized = true;
                    ...
                    let option = hostname
                        .option
                        .write(smoltcp::wire::DhcpOption { data, kind: 12 });
                    hostname.option_initialized = true;
    ...
    fn poll<D: Driver>(&mut self, cx: &mut Context<'_>, driver: &mut D) {
        ...
        if self.slaac && self.iface.slaac_updated_at() == timestamp {
            let ipv6_address = self.iface.ip_addrs().iter().find_map(|addr| match addr {
                IpCidr::Ipv6(ip6_address)
                    if !Ipv6Cidr::LINK_LOCAL_PREFIX.contains_addr(&ip6_address.address()) =>
                {
                    Some(ip6_address)
                }
                _ => None,
