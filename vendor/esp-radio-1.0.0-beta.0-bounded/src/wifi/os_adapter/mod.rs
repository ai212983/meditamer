...
pub unsafe extern "C" fn event_post(
    ...
    let callback = super::WifiCallbackGuard::enter_event();
...
const _: () =
    assert!(core::mem::size_of::<WifiStaticQueue>() == 2 * core::mem::size_of::<*mut c_void>());

pub unsafe extern "C" fn wifi_create_queue(queue_len: c_int, item_size: c_int) -> *mut c_void {
    let queue = WifiStaticQueue {
        handle: crate::compat::queue::queue_create(queue_len, item_size),
        // CompatQueue owns its guarded payload allocation. Preserve the vendor
        // ABI's second pointer even though no separate FreeRTOS storage exists.
        storage: core::ptr::null_mut(),
    };
    Box::leak(Box::new_in(queue, InternalMemory)) as *mut WifiStaticQueue as *mut c_void
}
...
pub unsafe extern "C" fn wifi_delete_queue(queue: *mut c_void) {
    let boxed = unsafe { Box::from_raw_in(queue.cast::<WifiStaticQueue>(), InternalMemory) };
    crate::compat::queue::queue_delete(boxed.handle)
}
