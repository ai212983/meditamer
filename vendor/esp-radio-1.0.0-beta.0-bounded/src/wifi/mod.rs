...
use portable_atomic::{AtomicBool, AtomicU8, AtomicU32, AtomicUsize, Ordering};
...
pub enum WifiError {
    ...
    /// Driver ownership could not be proved clean after initialization or shutdown failed.
    CleanupFailed,
...
pub(crate) fn wifi_init(_wifi: crate::hal::peripherals::WIFI<'_>) -> Result<(), WifiError> {
    ...
    unsafe {
        ...
        WIFI_CALLBACK_ADMISSION_OPEN.store(true, Ordering::Release);
        WIFI_CALLBACK_SOURCE_ACTIVE.store(true, Ordering::Release);
        Ok(())
    }
}
...
fn wifi_deinit() -> Result<(), crate::WifiError> {
    WIFI_CALLBACK_ADMISSION_OPEN.store(false, Ordering::Release);
    let stop_result = unsafe { esp_wifi_stop() } as u32;
    if stop_result != include::ESP_OK && stop_result != include::ESP_ERR_WIFI_NOT_STARTED {
        return Err(WifiError::from_error_code(stop_result as i32));
    }
    ...
    WIFI_CALLBACK_SOURCE_ACTIVE.store(false, Ordering::Release);
    Ok(())
}

static WIFI_CALLBACK_ADMISSION_OPEN: AtomicBool = AtomicBool::new(false);
static WIFI_CALLBACK_SOURCE_ACTIVE: AtomicBool = AtomicBool::new(false);
static WIFI_CALLBACK_IN_FLIGHT: AtomicU32 = AtomicU32::new(0);
static WIFI_CALLBACK_ACCEPTED: AtomicU32 = AtomicU32::new(0);
static WIFI_CALLBACK_LATE: AtomicU32 = AtomicU32::new(0);
const WIFI_RX_LIVE_COUNT_MASK: u32 = 0x0f;
const WIFI_RX_LIVE_PAYLOAD_SHIFT: u32 = 4;
static WIFI_RX_LIVE: AtomicU32 = AtomicU32::new(0);
static WIFI_RX_BUFFERS_PEAK: AtomicU32 = AtomicU32::new(0);
static WIFI_RX_PAYLOAD_BYTES_PEAK: AtomicU32 = AtomicU32::new(0);
static WIFI_RX_BUFFERS_CREATED: AtomicU32 = AtomicU32::new(0);
static WIFI_RX_BUFFERS_DROPPED: AtomicU32 = AtomicU32::new(0);
// Retain at most one station packet after callback entry and before queueing.
// A contending vendor callback may already own its vendor-allocated buffer;
// the product allocator hook accounts for that transient before this token can
// reject it.
static WIFI_RX_STA_CALLBACK_OWNED: AtomicBool = AtomicBool::new(false);

/// Wi-Fi callback-source state used by supervised radio teardown.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct WifiCallbackStats {
    /// Whether the vendor callback source is still registered and active.
    pub source_active: bool,
    /// Whether callbacks are still admitted into Wi-Fi-owned queues.
    pub admission_open: bool,
    /// Callback trampolines currently executing.
    pub in_flight: u32,
    /// Callbacks admitted since boot.
    pub accepted: u32,
    /// Callbacks observed after admission was revoked.
    pub late: u32,
    /// Vendor TX completions still outstanding.
    pub tx_in_flight: usize,
    /// A station RX callback currently owns a not-yet-queued packet.
    pub rx_sta_callback_owned: bool,
}

/// Return current and cumulative Wi-Fi callback-source counters.
pub fn wifi_callback_stats() -> WifiCallbackStats {
    WifiCallbackStats {
        source_active: WIFI_CALLBACK_SOURCE_ACTIVE.load(Ordering::Acquire),
        admission_open: WIFI_CALLBACK_ADMISSION_OPEN.load(Ordering::Acquire),
        in_flight: WIFI_CALLBACK_IN_FLIGHT.load(Ordering::Acquire),
        accepted: WIFI_CALLBACK_ACCEPTED.load(Ordering::Relaxed),
        late: WIFI_CALLBACK_LATE.load(Ordering::Relaxed),
        tx_in_flight: WIFI_TX_INFLIGHT.load(Ordering::Acquire),
        rx_sta_callback_owned: WIFI_RX_STA_CALLBACK_OWNED.load(Ordering::Acquire),
    }
}

/// Dynamic vendor RX-buffer ownership used to correlate internal-heap low water.
#[derive(Clone, Copy, Debug, Default, PartialEq, Eq)]
pub struct WifiRxBufferStats {
    /// Packet buffers currently owned by the first-party RX queues or network stack.
    pub live: u32,
    /// Maximum simultaneously live packet buffers since boot.
    pub peak: u32,
    /// Sum of received payload lengths for the currently live packet buffers.
    pub payload_bytes_live: u32,
    /// Maximum simultaneous live payload bytes since boot.
    pub payload_bytes_peak: u32,
    /// Packet buffers received from the vendor callback since boot.
    pub created: u32,
    /// Packet buffers returned to the vendor driver since boot.
    pub dropped: u32,
}

/// Return current and cumulative dynamic vendor RX-buffer ownership.
pub fn wifi_rx_buffer_stats() -> WifiRxBufferStats {
    let live = WIFI_RX_LIVE.load(Ordering::Acquire);
    WifiRxBufferStats {
        live: live & WIFI_RX_LIVE_COUNT_MASK,
        peak: WIFI_RX_BUFFERS_PEAK.load(Ordering::Relaxed),
        payload_bytes_live: live >> WIFI_RX_LIVE_PAYLOAD_SHIFT,
        payload_bytes_peak: WIFI_RX_PAYLOAD_BYTES_PEAK.load(Ordering::Relaxed),
        created: WIFI_RX_BUFFERS_CREATED.load(Ordering::Relaxed),
        dropped: WIFI_RX_BUFFERS_DROPPED.load(Ordering::Relaxed),
    }
}

fn record_wifi_rx_buffer_created(len: u16) {
    let (live, payload_live) = WIFI_RX_LIVE
        .fetch_update(Ordering::AcqRel, Ordering::Acquire, |packed| {
            let live = packed & WIFI_RX_LIVE_COUNT_MASK;
            let payload = packed >> WIFI_RX_LIVE_PAYLOAD_SHIFT;
            assert!(live < WIFI_RX_LIVE_COUNT_MASK, "Wi-Fi RX live count overflow");
            let payload = payload
                .checked_add(len as u32)
                .expect("Wi-Fi RX live payload overflow");
            Some((payload << WIFI_RX_LIVE_PAYLOAD_SHIFT) | (live + 1))
        })
        .map(|previous| {
            let updated = previous
                + (1 | ((len as u32) << WIFI_RX_LIVE_PAYLOAD_SHIFT));
            (
                updated & WIFI_RX_LIVE_COUNT_MASK,
                updated >> WIFI_RX_LIVE_PAYLOAD_SHIFT,
            )
        })
        .expect("Wi-Fi RX live ownership update cannot fail");
    WIFI_RX_BUFFERS_PEAK.fetch_max(live, Ordering::Relaxed);
    WIFI_RX_PAYLOAD_BYTES_PEAK.fetch_max(payload_live, Ordering::Relaxed);
    WIFI_RX_BUFFERS_CREATED.fetch_add(1, Ordering::Relaxed);
}

fn record_wifi_rx_buffer_dropped(len: u16) {
    WIFI_RX_LIVE
        .fetch_update(Ordering::AcqRel, Ordering::Acquire, |packed| {
            let live = packed & WIFI_RX_LIVE_COUNT_MASK;
            let payload = packed >> WIFI_RX_LIVE_PAYLOAD_SHIFT;
            assert!(live > 0, "Wi-Fi RX live count underflow");
            let payload = payload
                .checked_sub(len as u32)
                .expect("Wi-Fi RX live payload underflow");
            Some((payload << WIFI_RX_LIVE_PAYLOAD_SHIFT) | (live - 1))
        })
        .expect("Wi-Fi RX live ownership update cannot fail");
    WIFI_RX_BUFFERS_DROPPED.fetch_add(1, Ordering::Relaxed);
}

struct WifiCallbackGuard {
    admitted: bool,
}

impl WifiCallbackGuard {
    fn enter() -> Self {
        WIFI_CALLBACK_IN_FLIGHT.fetch_add(1, Ordering::AcqRel);
        let admitted = WIFI_CALLBACK_ADMISSION_OPEN.load(Ordering::Acquire);
        if admitted {
            WIFI_CALLBACK_ACCEPTED.fetch_add(1, Ordering::Relaxed);
        } else {
            WIFI_CALLBACK_LATE.fetch_add(1, Ordering::Relaxed);
        }
        Self { admitted }
    }
    ...
    fn enter_event() -> Self {
        WIFI_CALLBACK_IN_FLIGHT.fetch_add(1, Ordering::AcqRel);
        // Stop/disconnect events remain valid while the vendor source is being
        // synchronously deinitialized, even after packet admission is closed.
        // Once the source is disabled, later events are counted and suppressed.
        let admitted = WIFI_CALLBACK_SOURCE_ACTIVE.load(Ordering::Acquire);
        if admitted {
            WIFI_CALLBACK_ACCEPTED.fetch_add(1, Ordering::Relaxed);
        } else {
            WIFI_CALLBACK_LATE.fetch_add(1, Ordering::Relaxed);
...
impl Drop for WifiCallbackGuard {
    fn drop(&mut self) {
        WIFI_CALLBACK_IN_FLIGHT.fetch_sub(1, Ordering::AcqRel);
    }
}
...
unsafe extern "C" fn recv_cb_sta(
        ...
        .is_err()
    {
        ...
        return include::ESP_ERR_NO_MEM as esp_err_t;
    }
    struct CallbackOwner;
    impl Drop for CallbackOwner {
        fn drop(&mut self) {
            WIFI_RX_STA_CALLBACK_OWNED.store(false, Ordering::Release);
        }
    }
    let _callback_owner = CallbackOwner;
    let packet = PacketBuffer::new(buffer, len, eb);
...
unsafe extern "C" fn esp_wifi_tx_done_cb(
    ...
    let _callback = WifiCallbackGuard::enter();
...
mod private {
    ...
    impl Drop for PacketBuffer {
        fn drop(&mut self) {
            ...
            // Retain ownership in telemetry until the vendor allocator has
            // actually completed the free.
            record_wifi_rx_buffer_dropped(self.len);
        }
    }
    ...
    impl PacketBuffer {
        pub(super) fn new(
            ...
            record_wifi_rx_buffer_created(len);
            Self { buffer, len, eb }
        }
...
pub struct WifiController<'d> {
    guard: Option<RadioRefGuard>,
    queue_epoch: WifiQueueEpochGuard,
    driver_deinitialized: bool,
    shutdown_complete: bool,
    _phantom: PhantomData<&'d ()>,
}

#[derive(Debug)]
struct WifiQueueEpochGuard {
    reclaimed: bool,
}

impl WifiQueueEpochGuard {
    fn begin() -> Self {
        let epoch = crate::compat::queue_lifecycle::begin_reclaimable_epoch();
        debug!("starting Wi-Fi queue owner epoch {}", epoch);
        Self { reclaimed: false }
    }

    fn reclaim(&mut self) -> Result<usize, crate::compat::queue_lifecycle::ReclaimError> {
        if self.reclaimed {
            return Ok(0);
        }
        let reclaimed =
            crate::compat::queue_lifecycle::reclaim_current_epoch_after_source_quiescent()?;
        self.reclaimed = true;
        debug!("reclaimed {} Wi-Fi queue owner slots", reclaimed);
        Ok(reclaimed)
    }

    fn cleanup_before_driver_init(&mut self, original: WifiError) -> WifiError {
        let callbacks = wifi_callback_stats();
        if !callbacks.source_active
...
impl Drop for WifiController<'_> {
    fn drop(&mut self) {
        ...
        if let Err(error) = self.shutdown_source() {
            warn!("Failed to cleanly deinit wifi: {:?}", error);
            return;
        }
        if wifi_callback_stats().in_flight == 0 && wifi_callback_stats().tx_in_flight == 0 {
            if let Err(error) = self.finalize_shutdown() {
                warn!("Failed to reclaim Wi-Fi queues: {:?}", error);
            }
        }
    }
}
...
impl<'d> WifiController<'d> {
    /// Disable the vendor Wi-Fi callback source without reclaiming queue storage.
    pub fn shutdown_source(&mut self) -> Result<(), WifiError> {
        if self.driver_deinitialized {
            return Ok(());
        }
        state::locked(|| {
        ...
        self.driver_deinitialized = true;
        Ok(())
    }

    /// Release the radio guard and reclaim queues after callback quiescence.
    pub fn finalize_shutdown(&mut self) -> Result<(), WifiError> {
        if self.shutdown_complete {
            return Ok(());
        }
        let callbacks = wifi_callback_stats();
        if !self.driver_deinitialized
            || callbacks.source_active
            || callbacks.in_flight != 0
            || callbacks.tx_in_flight != 0
            || callbacks.rx_sta_callback_owned
        {
            return Err(WifiError::CleanupFailed);
        }
        drop(self.guard.take());
        self.queue_epoch
            .reclaim()
            .map_err(|_| WifiError::CleanupFailed)?;
    ...
    pub fn new(
        ...
        // Local variables drop in reverse order. Starting the queue epoch first
        // makes every construction-error path disable radio sources before it
        // attempts source-quiescent reclamation.
        let mut queue_epoch = WifiQueueEpochGuard::begin();
        ...
        {
            drop(_guard);
            return Err(queue_epoch.cleanup_before_driver_init(error));
        }
        if let Err(error) =
        ...
        if let Err(error) = crate::wifi::wifi_init(device) {
            let cleanup_ok = crate::wifi::wifi_deinit().is_ok();
            drop(_guard);
            let callbacks = wifi_callback_stats();
            if cleanup_ok
                && !callbacks.source_active
        ...
        let mut controller = WifiController {
            guard: Some(_guard),
            queue_epoch,
            driver_deinitialized: false,
            shutdown_complete: false,
            _phantom: Default::default(),
        };

        if let Err(error) = controller.set_country_info(&config.country_info) {
            return Err(controller.cleanup_construction_error(error));
        }
        // Set a sane default power saving mode. The blob default is not the best for bandwidth.
        if let Err(error) = controller.set_power_saving(PowerSaveMode::default()) {
            return Err(controller.cleanup_construction_error(error));
        }

        if let Err(error) = controller.set_config(&config.initial_config) {
            return Err(controller.cleanup_construction_error(error));
        }
    ...
    fn cleanup_construction_error(&mut self, original: WifiError) -> WifiError {
        if self.shutdown_source().is_ok()
            && wifi_callback_stats().in_flight == 0
            && wifi_callback_stats().tx_in_flight == 0
            && self.finalize_shutdown().is_ok()
        {
            original
        } else {
            WifiError::CleanupFailed
