...
#[cfg(feature = "esp-radio")]
pub(crate) const TASK_ROLE_LEN: usize = 24;

#[cfg(feature = "esp-radio")]
fn write_task_role(dst: &mut [u8; TASK_ROLE_LEN], role: &str) {
...
pub(crate) struct Task {
    ...
    #[cfg(feature = "esp-radio")]
    pub task_role: [u8; TASK_ROLE_LEN],
    pub state: TaskState,
...
impl Task {
    ...
    pub(crate) fn new(
        ...
        #[cfg(feature = "esp-radio")]
        write_task_role(&mut task.task_role, name);
...
pub(super) fn allocate_main_task(
    ...
    #[cfg(feature = "esp-radio")]
    write_task_role(&mut scheduler.per_cpu[current_cpu].main_task.task_role, "main");
    #[cfg(multi_core)]
    {
    ...
    #[cfg(feature = "esp-radio")]
    if super::scheduler::ESP_RTOS_TASK_CREATE_DIAG {
        warn!(
            "upload_http: esp_rtos_task_create name=main ptr=0x{:x} priority=0 pinned_to={:?}",
            main_task_ptr.as_ptr() as usize,
            Some(cpu),
        );
    }
...
#[cfg(feature = "esp-radio")]
#[unsafe(no_mangle)]
pub extern "C" fn esp_rtos_task_role(task: *const Task) -> *const core::ffi::c_char {
    if task.is_null() {
