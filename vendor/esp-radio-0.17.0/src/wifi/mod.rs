...
pub(crate) mod state;
pub(crate) use internal::WifiInitConfigDiag;
...
#[cfg(esp32)]
pub(crate) fn legacy_phy_mem_init_diag() {
    phy_legacy_esp32::phy_mem_init_diag();
}

const MTU: usize = esp_config_int!(usize, "ESP_RADIO_CONFIG_WIFI_MTU");

const fn wifi_init_trace_enabled() -> bool {
    matches!(option_env!("MEDITAMER_WIFI_ESP_RADIO_INIT_TRACE"), Some(_))
        || matches!(option_env!("ESP_RADIO_INIT_TRACE"), Some(_))
}

const fn wifi_use_idf_init_defaults_enabled() -> bool {
    matches!(
        option_env!("MEDITAMER_WIFI_ESP_RADIO_USE_IDF_INIT_DEFAULTS"),
        Some(_)
    ) || matches!(option_env!("ESP_RADIO_USE_IDF_INIT_DEFAULTS"), Some(_))
}

const fn wifi_use_idf_init_defaults_no_nvs_enabled() -> bool {
    matches!(
        option_env!("MEDITAMER_WIFI_ESP_RADIO_USE_IDF_INIT_DEFAULTS_NO_NVS"),
        Some(_)
    ) || matches!(option_env!("ESP_RADIO_USE_IDF_INIT_DEFAULTS_NO_NVS"), Some(_))
}

const fn wifi_use_idf_like_init_defaults_enabled() -> bool {
    wifi_use_idf_init_defaults_enabled() || wifi_use_idf_init_defaults_no_nvs_enabled()
}

const fn wifi_use_idf_task_core_enabled() -> bool {
    matches!(option_env!("MEDITAMER_WIFI_ESP_RADIO_USE_IDF_TASK_CORE"), Some(_))
        || matches!(option_env!("ESP_RADIO_USE_IDF_TASK_CORE"), Some(_))
}
...
const fn wifi_skip_mac_time_update_cb_diag_enabled() -> bool {
    matches!(
        option_env!("MEDITAMER_WIFI_ESP_RADIO_SKIP_MAC_TIME_UPDATE_CB_DIAG"),
        Some(_)
    ) || matches!(
        option_env!("ESP_RADIO_SKIP_MAC_TIME_UPDATE_CB_DIAG"),
        Some(_)
    )
}
...
pub(crate) fn wifi_init(_wifi: crate::hal::peripherals::WIFI<'_>) -> Result<(), WifiError> {
    wifi_init_trace("begin");
    #[cfg(esp32)]
    {
        if wifi_skip_mac_time_update_cb_diag_enabled() {
            wifi_init_trace("set_mac_time_update_cb.skipped");
        } else {
            wifi_init_trace("set_mac_time_update_cb.before");
            set_mac_time_update_cb(_wifi);
            wifi_init_trace("set_mac_time_update_cb.after");
        }
    }
    unsafe {
        #[cfg(coex)]
        {
            wifi_init_trace("coex_init.before");
            esp_wifi_result!(coex_init())?;
            wifi_init_trace("coex_init.after");
        }

        wifi_init_trace("esp_wifi_init_internal.before");
        esp_wifi_result!(esp_wifi_init_internal(addr_of!(internal::G_CONFIG)))?;
        wifi_init_trace("esp_wifi_init_internal.after");

        wifi_init_trace("esp_wifi_set_mode_null.before");
        esp_wifi_result!(esp_wifi_set_mode(wifi_mode_t_WIFI_MODE_NULL))?;
        wifi_init_trace("esp_wifi_set_mode_null.after");

        wifi_init_trace("esp_supplicant_init.before");
        esp_wifi_result!(esp_supplicant_init())?;
        wifi_init_trace("esp_supplicant_init.after");

        wifi_init_trace("esp_wifi_set_tx_done_cb.before");
...
pub(crate) fn wifi_init_config_diag() -> WifiInitConfigDiag {
    if crate::compat::legacy_runtime_policy::backend_legacy_port_enabled() {
        ...
        return WifiInitConfigDiag {
            ...
            magic: diag.magic,
            osi_set_intr_ptr: diag.osi_set_intr_ptr,
            osi_clear_intr_ptr: diag.osi_clear_intr_ptr,
            osi_set_isr_ptr: diag.osi_set_isr_ptr,
            osi_ints_on_ptr: diag.osi_ints_on_ptr,
            osi_ints_off_ptr: diag.osi_ints_off_ptr,
            osi_wifi_int_disable_ptr: diag.osi_wifi_int_disable_ptr,
            osi_wifi_int_restore_ptr: diag.osi_wifi_int_restore_ptr,
            osi_task_yield_from_isr_ptr: diag.osi_task_yield_from_isr_ptr,
            osi_queue_create_ptr: diag.osi_queue_create_ptr,
            osi_queue_send_from_isr_ptr: diag.osi_queue_send_from_isr_ptr,
...
#[doc(hidden)]
pub fn diagnostic_reset_wifi_rx_cb_counts() {
    WIFI_RX_CB_STA_COUNT.store(0, Ordering::Relaxed);
    WIFI_RX_CB_AP_COUNT.store(0, Ordering::Relaxed);
}

#[doc(hidden)]
pub fn diagnostic_wifi_rx_cb_counts() -> (usize, usize) {
    (
...
pub(crate) fn wifi_start_scan(
    ...
    scan_type.validate();
    let use_idf_default_scan_timing = wifi_use_idf_default_scan_timing_diag_enabled()
        && matches!(
            scan_type,
            ScanTypeConfig::Active { min, max }
                if min == Duration::from_millis(10) && max == Duration::from_millis(20)
        );
    let (scan_time, scan_type, home_chan_dwell_time) = match scan_type {
        ScanTypeConfig::Active { min, max } => {
            let (min_ms, max_ms, home_dwell_ms) = if use_idf_default_scan_timing {
                (0, 120, 30)
            } else {
                (min.as_millis() as u32, max.as_millis() as u32, 0)
            };
            (
                wifi_scan_time_t {
...
pub fn new<'d>(
    ...
    wifi_new_trace("begin");
    if crate::is_interrupts_disabled() {
        return Err(WifiError::Unsupported);
    }

    config.validate();
    wifi_new_trace("config_validated");
    ...
    unsafe {
        internal::G_CONFIG = wifi_init_config_t {
            ...
            nvs_enable: if wifi_use_idf_init_defaults_enabled() {
                esp_wifi_sys::include::WIFI_NVS_ENABLED as i32
            } else {
                0
            },
            nano_enable: if wifi_use_idf_like_init_defaults_enabled() {
                esp_wifi_sys::include::WIFI_NANO_FORMAT_ENABLED as i32
            } else {
                0
            },
            rx_ba_win: config.rx_ba_win as _,
            wifi_task_core_id: if wifi_use_idf_like_init_defaults_enabled()
                || wifi_use_idf_task_core_enabled()
            {
                esp_wifi_sys::include::WIFI_TASK_CORE_ID as i32
            } else {
                Cpu::current() as _
            },
            ...
            sta_disconnected_pm: if wifi_use_idf_like_init_defaults_enabled() {
                esp_wifi_sys::include::WIFI_STA_DISCONNECTED_PM_ENABLED != 0
            } else {
                false
            },
            ...
            tx_hetb_queue_num: if wifi_use_idf_like_init_defaults_enabled() {
                esp_wifi_sys::include::WIFI_TX_HETB_QUEUE_NUM as i32
            } else {
                3
            },
            dump_hesigb_enable: if wifi_use_idf_like_init_defaults_enabled() {
                esp_wifi_sys::include::WIFI_DUMP_HESIGB_ENABLED != 0
            } else {
                false
            },
    ...
    wifi_new_trace("g_config_written");

    wifi_new_trace("wifi_init.before");
    crate::wifi::wifi_init(device)?;
    wifi_new_trace("wifi_init.after");

    unsafe {
        wifi_new_trace("set_country.before");
        let country = config.country_code.into_blob();
        esp_wifi_result!(esp_wifi_set_country(&country))?;
        wifi_new_trace("set_country.after");
    }
    ...
    wifi_new_trace("trng_entropy_enabled");
    ...
    wifi_new_trace("controller_created");

    wifi_new_trace("set_power_saving.before");
    controller.set_power_saving(config.power_save_mode)?;
    wifi_new_trace("set_power_saving.after");
