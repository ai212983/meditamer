...
pub(super) static mut BLOB_COMPAT_G_WIFI_FEATURE_CAPS: u64 = WIFI_FEATURE_CAPS;
...
pub(super) unsafe fn install_blob_compat_g_config(
    config: Config,
    ...
    G_CONFIG = wifi_init_config_t {
        ...
        static_rx_buf_num: config.static_rx_buf_num as i32,
        dynamic_rx_buf_num: config.dynamic_rx_buf_num as i32,
        tx_buf_type: esp_wifi_sys::include::CONFIG_ESP_WIFI_TX_BUFFER_TYPE as i32,
        static_tx_buf_num: config.static_tx_buf_num as i32,
        dynamic_tx_buf_num: config.dynamic_tx_buf_num as i32,
        rx_mgmt_buf_type: esp_wifi_sys::include::CONFIG_ESP_WIFI_DYNAMIC_RX_MGMT_BUF as i32,
        rx_mgmt_buf_num: esp_wifi_sys::include::CONFIG_ESP_WIFI_RX_MGMT_BUF_NUM_DEF as i32,
        cache_tx_buf_num: esp_wifi_sys::include::WIFI_CACHE_TX_BUFFER_NUM as i32,
        csi_enable: cfg!(feature = "csi") as i32,
        ampdu_rx_enable: config.ampdu_rx_enable as i32,
        ampdu_tx_enable: config.ampdu_tx_enable as i32,
        amsdu_tx_enable: config.amsdu_tx_enable as i32,
        nvs_enable: 0,
        nano_enable: 0,
        rx_ba_win: config.rx_ba_win as i32,
        wifi_task_core_id: 0,
        beacon_max_len: esp_wifi_sys::include::WIFI_SOFTAP_BEACON_MAX_LEN as i32,
        mgmt_sbuf_num: esp_wifi_sys::include::WIFI_MGMT_SBUF_NUM as i32,
        feature_caps: BLOB_COMPAT_G_WIFI_FEATURE_CAPS,
        sta_disconnected_pm: false,
...
pub(super) unsafe fn install_blob_compat_globals(
    config: Config,
    wpa_crypto: wpa_crypto_funcs_t,
) {
    install_blob_compat_g_config(config, wpa_crypto);
}

#[derive(Clone, Copy, Debug, Default)]
pub struct WifiInitConfigDiag {
    pub config_ptr: usize,
    pub osi_funcs_ptr: usize,
    pub static_rx_buf_num: i32,
    pub dynamic_rx_buf_num: i32,
    pub static_tx_buf_num: i32,
    pub dynamic_tx_buf_num: i32,
    pub rx_mgmt_buf_type: i32,
    pub rx_mgmt_buf_num: i32,
    ...
    pub osi_task_get_current_ptr: usize,
    pub osi_wifi_thread_semphr_get_ptr: usize,
    pub osi_timer_arm_us_ptr: usize,
    pub osi_event_post_ptr: usize,
    pub osi_malloc_internal_ptr: usize,
}
...
pub(crate) fn wifi_init_config_diag() -> WifiInitConfigDiag {
    let config_ptr = (&raw const G_CONFIG) as usize;
    let config = unsafe { &*(&raw const G_CONFIG) };
    let osi_funcs_ptr = config.osi_funcs as usize;
    let osi = if osi_funcs_ptr == 0 {
        None
    } else {
        Some(unsafe { &*config.osi_funcs })
    };

    WifiInitConfigDiag {
        config_ptr,
        osi_funcs_ptr,
        static_rx_buf_num: config.static_rx_buf_num,
        dynamic_rx_buf_num: config.dynamic_rx_buf_num,
        static_tx_buf_num: config.static_tx_buf_num,
        dynamic_tx_buf_num: config.dynamic_tx_buf_num,
        rx_mgmt_buf_type: config.rx_mgmt_buf_type,
        rx_mgmt_buf_num: config.rx_mgmt_buf_num,
        cache_tx_buf_num: config.cache_tx_buf_num,
        ampdu_rx_enable: config.ampdu_rx_enable,
        ampdu_tx_enable: config.ampdu_tx_enable,
        amsdu_tx_enable: config.amsdu_tx_enable,
        nvs_enable: config.nvs_enable,
        nano_enable: config.nano_enable,
        rx_ba_win: config.rx_ba_win,
        wifi_task_core_id: config.wifi_task_core_id,
        feature_caps: config.feature_caps,
        sta_disconnected_pm: config.sta_disconnected_pm,
        ...
        osi_task_get_current_ptr: osi.map_or(0, |v| fn_ptr_addr!(v._task_get_current_task)),
        osi_wifi_thread_semphr_get_ptr: osi
            .map_or(0, |v| fn_ptr_addr!(v._wifi_thread_semphr_get)),
        osi_timer_arm_us_ptr: osi.map_or(0, |v| fn_ptr_addr!(v._timer_arm_us)),
        osi_event_post_ptr: osi.map_or(0, |v| fn_ptr_addr!(v._event_post)),
        osi_malloc_internal_ptr: osi.map_or(0, |v| fn_ptr_addr!(v._malloc_internal)),
    }
}
