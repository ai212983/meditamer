...
pub struct CallbackRouteTable<T: Copy, const CAPACITY: usize> {
...
impl<T: Copy, const CAPACITY: usize> CallbackRouteTable<T, CAPACITY> {
    ...
    pub fn claim(&mut self, value: T) -> Result<CallbackRoute, CallbackRouteError> {
    ...
    pub fn enable(&mut self, route: CallbackRoute) -> Result<(), CallbackRouteError> {
        self.slot_mut(route)?.enabled = true;
        Ok(())
    }

    pub fn disable(&mut self, route: CallbackRoute) -> Result<(), CallbackRouteError> {
        self.slot_mut(route)?.enabled = false;
        Ok(())
    }

    pub fn release(&mut self, route: CallbackRoute) -> Result<(), CallbackRouteError> {
    ...
    pub fn resolve(&self, route: CallbackRoute) -> Option<T> {
    ...
    pub fn any_value(&self, mut matches: impl FnMut(T) -> bool) -> bool {
        self.slots
            .iter()
...
impl<T: Copy, const CAPACITY: usize> Default for CallbackRouteTable<T, CAPACITY> {
    fn default() -> Self {
        Self::new()
    }
}
