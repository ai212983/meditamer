...
if [[ "$mode" == "staged" ]]; then
  while IFS= read -r -d '' file; do
    ...
    files+=("$file")
  done < <(git diff --cached --name-only --diff-filter=ACMRTUXB -z -- src packages tools test-support)
else
  while IFS= read -r -d '' file; do
    [[ "$file" == *.rs ]] || continue
    [[ -f "$file" ]] || continue
    files+=("$file")
  done < <(git ls-files -z -- src packages tools test-support)
fi
...
for file in "${files[@]}"; do
  ...
  if [[ "$mode" == "staged" ]]; then
    source_command=(git cat-file blob ":$file")
  else
    source_command=(cat -- "$file")
  fi
  while IFS= read -r hit; do
    ...
    printf '%s\t%s\n' "$file" "$hit" >>"$tmp_bad"
  done < <("${source_command[@]}" | grep -n 'include!' || true)
done
