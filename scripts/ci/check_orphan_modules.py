...
FIRST_PARTY_PREFIXES = ("src/", "packages/", "tools/")
...
def git_paths(repo_root: Path, *pathspecs: str) -> list[Path]:
...
def existing_tracked_rust(repo_root: Path) -> set[Path]:
    return {
        path.resolve()
        for path in git_paths(repo_root, "src", "packages", "tools", "build.rs")
        if path.suffix == ".rs" and path.is_file() and is_first_party(path, repo_root)
    }


def manifest_paths(repo_root: Path) -> list[Path]:
    paths = git_paths(repo_root, "Cargo.toml", "packages", "tools")
    return sorted(
        path.resolve()
        for path in paths
        if path.name == "Cargo.toml" and path.is_file() and is_first_party(path, repo_root)
    )
...
def main() -> int:
    ...
    tracked = existing_tracked_rust(repo_root)
    roots: list[TargetRoot] = []
    candidates: set[Path] = set()
    ...
    for orphan in orphans:
        print(f"  - {orphan.relative_to(repo_root).as_posix()}", file=sys.stderr)
    return 1
