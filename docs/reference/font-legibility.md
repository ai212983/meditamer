# Font legibility on 1-bit / 3-bit displays

Curated external references for improving text legibility on low-bit-depth
displays, annotated for compatibility with our LVGL font pipeline, plus the
current state of the UI font pipeline itself.

Read this before touching fonts, the dither threshold, or `lv_conf.h` font
config. Unstarted work derived from this reference lives in
[`plans/font-legibility-improvements.md`](../plans/font-legibility-improvements.md).
...
- `assets/fonts/IBMPlexSans-Variable.ttf` feeds one compiled face: the 128 px Ambient
  Home clock (digits, colon, dash), built by
  [`tools/lvgl_font_compiler`](../../tools/lvgl_font_compiler/src/lib.rs) from `build.rs`
  into `OUT_DIR` at `--bpp 1`. LVGL's built-in Montserrat tables stop at 48 px, so a
  clock that large has to be compiled. 1 bpp because the flush path thresholds at 128:
  an anti-aliased ramp would quadruple the table without surviving the threshold.
...
## Current UI font state

Both pages use LVGL's built-in **Montserrat** only, at four sizes:

| Screen | Element | Font |
| --- | --- | --- |
| Home | "Meditamer" title | montserrat_24 (`home.rs`) |
| Home | "Ready" status | montserrat_18 (`home.rs`) |
| Home | arrows hint | montserrat_18 (`home.rs`) |
| Home | "TOP TEST" button | montserrat_18 (`home.rs`) |
| Gesture test | title | montserrat_24 (`gesture_test.rs`) |
| Gesture test | instructions | montserrat_18 (`gesture_test.rs`) |
| Gesture test | result panel | montserrat_20 (`gesture_test.rs`) |
| Both (carousel) | `<` / `>` buttons | montserrat_32 (`carousel.rs`) |
| Both (carousel) | page indicator | montserrat_18 (`carousel.rs`) |
