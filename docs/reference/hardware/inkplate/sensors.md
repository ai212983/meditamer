...
| Memory | Physical capacity | Meditamer-visible capacity / role |
| --- | ---: | --- |
| Internal SRAM | 520 KiB | Split between instruction RAM, data RAM, caches, and system reservations. The current linker exposes 310,448 bytes across its two internal data-RAM windows; see the [DRAM budget](../../dram/dram-budget.md). |
| RTC SRAM | 16 KiB | 8 KiB RTC FAST plus 8 KiB RTC SLOW memory; separate from the 520 KiB main SRAM. |
| Mask ROM | 448 KiB | Fixed boot and core routines; read-only and not application or asset storage. |
| External SPI flash | 4 MiB | Persistent bootloader, metadata, state, and firmware storage. The accepted layout has one 3,670,016-byte production application slot (`ota_0`) plus a 393,216-byte factory updater slot; see the [single-production factory updater decision](../../../architecture/0014-single-production-sd-recovery-updater.md) (supersedes the earlier [two-slot A/B decision](../../../architecture/0009-ab-firmware-update-foundation.md)). |
| External PSRAM | 8 MiB physical | The original ESP32 maps 4 MiB directly, which is the capacity exposed to Meditamer's ordinary PSRAM allocator. Access to the upper 4 MiB would require bank switching and is not currently implemented. |

The silicon totals come from Espressif's [ESP32 series datasheet](https://documentation.espressif.com/esp32_datasheet_en.html). The module capacities come from the [ESP32-WROVER-E datasheet](https://documentation.espressif.com/esp32-wrover-e_esp32-wrover-ie_datasheet_en.html). An exact-board ESP-IDF control run detected the 8 MiB PSRAM device while adding a 4 MiB pool to the ordinary allocator; that evidence is retained in the [PSRAM investigation](../../../archive/wifi/wifi-psram-discovery-validation-plan.md#phase-3-official-esp-idf-control). Espressif documents the 4 MiB mapping limit and access to the upper bank in its [Himem guide](https://docs.espressif.com/projects/esp-idf/en/stable/esp32/api-reference/system/himem.html).

These capacities are not one combined heap: ROM is immutable, flash is persistent storage, internal SRAM is divided among code/data/system uses, and PSRAM is external volatile working memory.
