...
67. [x] Add direct-mode repeated-reset degradation guard: when direct `PUT /upload` transport-reset streak exceeds configured limit, trigger same-cycle chunked fallback (`upload_begin/chunk/commit`) instead of failing cycle; keep marker telemetry in host diagnostics (`transport_reset_chunk_fallback_trigger` + `host_upload_transport_fallback`) and smoke-validate in direct mode (`logs/adhoc_directfallback10_20260305_1602.log`, `logs/adhoc_directfallback10_20260305_1602.log.hostdiag`).
