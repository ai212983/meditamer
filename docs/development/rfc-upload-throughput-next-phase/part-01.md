...
69. [x] Harden acceptance retry behavior for host-side send/reset turbulence by enabling fresh-client refresh retry by default (`HOSTCTL_NET_UPLOAD_REFRESH_ON_FAILURE=1`) and expanding retry-eligible classes to include `host_transport_connection_reset` in addition to send/health-send failures (unit-validated; live bounded gate attempt blocked pre-upload by `listener_not_ready`: `logs/wifi_regression_gate_20260305_161739/report.json`).
70. [x] Close `listener_not_ready` recurrence in host acceptance startup path by adding pre-run state normalization (`STATE SET upload=on`, `STATE DIAG kind=NONE targets=NONE`), listener-ready grace probe (`HOSTCTL_NET_LISTENER_READY_GRACE_MS=2000`), and stronger recover sequence (`NET STOP` before `NET RECOVER` when forcing start recovery); validated with two consecutive bounded regression-gate passes (`logs/wifi_regression_gate_20260305_165416/report.json`, `logs/wifi_regression_gate_20260305_165808/report.json`).
71. [x] Add bounded `net_wait_ready` recover-retry hardening in acceptance startup (`HOSTCTL_NET_WAIT_READY_RECOVER_RETRIES=1` default) for listener/DHCP stall signatures, then validate on live hardware: retry path triggers as expected, but terminal `listener_not_ready` remains reproducible after retry in quick bounded run (`logs/wifi_acceptance_waitretry_quick_20260305_173625.stdout.log`, `logs/wifi_acceptance_waitretry_quick_20260305_173625.log`); next focus remains firmware-side DHCP/listener lease-path root cause under AP contention.
72. [x] Validate firmware lease-status synchronization and `dhcp_ready` precondition telemetry on flashed firmware (`success_progress` lease/IP sync), then reproduce failing listener-enabled acceptance path to verify whether `ListenerWait` status keeps non-zero lease IP when lease is present and to separate true lease loss from listener-arm delay (post-flash strict acceptance repro confirms repeated `dhcp_ready` with `lease_ipv4=192.168.114.105` and sustained `ListenerWait` statuses with `ipv4=192.168.114.105`, while listener remains false until `listener_timeout`: `logs/wifi_regression_gate_20260305_175630/acceptance_1_cycle.log`; discovery marker sanity still captured in `logs/wifi_discovery_leasecheck_postflash_1r_20260305_175254.log`).
73. [x] Run a listener-arm root-cause pass with new HTTP server-loop gate diagnostics (`transfers_paused` + `listener_gate reason/clear` traces) and strict gate profile, then classify whether the dominant pre-timeout stall is transfer-phase suppression (`DiagnosticsExclusive`/upload service gating) or DHCP/listener gate churn (`wifi_down`/`link_down`/`no_ipv4`) under AP contention (strict soak-skipped reruns first passed without recurrence: `logs/wifi_regression_gate_20260305_180911/report.json`, `logs/wifi_regression_gate_20260305_182902/report.json`, `logs/wifi_regression_gate_20260305_183347/report.json`; then a failing window captured repeated `ListenerWait -> listener_timeout` loops with non-zero lease and terminal `listener_not_ready` in `logs/wifi_regression_batch_20260305_185110/run_1/acceptance_1_cycle.log`, pointing to stale listener accept path rather than transfer-phase suppression. Firmware mitigation landed in HTTP accept loop gate-aware polling/re-arm path (`src/firmware/storage/upload/http/socket_cycle.rs`) and two post-flash strict gates passed with zero listener-timeout events in acceptance logs: `logs/wifi_regression_gate_20260305_190833/report.json`, `logs/wifi_regression_gate_20260305_191234/report.json`).
74. [x] Run a bounded AP-contention confirmation batch (strict profile, soak skipped) focused on listener stability tails (`listener_timeout`, `listener_not_ready`, `accept_link_reset`) and internal-free drift slope, then decide whether to promote this listener re-arm mitigation as closed/hardened or continue root-cause work (targeted loop hardening landed: listener-timeout streak/internal-free-drop guard now escalates to stop/start hard recover before budget exhaustion; follow-up AP-contention batch `logs/wifi_apcontention_confirm_20260305_193824/summary.tsv` completed 3/3 passed with `listener_timeout=0`, `listener_not_ready=0`, `accept_link_reset=0`, and `lw_drift_a1/a3=0` for all runs. Decision: promote listener re-arm + loop hardening as hardened for this failure class; keep guard trigger-path validation as non-blocking follow-up).
