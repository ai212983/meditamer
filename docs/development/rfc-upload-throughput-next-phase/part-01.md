...
69. [x] Harden acceptance retry behavior for host-side send/reset turbulence by enabling fresh-client refresh retry by default (`HOSTCTL_NET_UPLOAD_REFRESH_ON_FAILURE=1`) and expanding retry-eligible classes to include `host_transport_connection_reset` in addition to send/health-send failures.
