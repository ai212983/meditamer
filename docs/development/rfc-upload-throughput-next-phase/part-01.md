...
70. [ ] Root-cause `listener_not_ready` recurrence where acceptance cycles stall in `ListenerWait` with `link=true` and `ipv4=0.0.0.0` after `dhcp_ready`, then fail terminally; collect targeted DHCP/listener transition diagnostics and patch readiness/state handoff.
