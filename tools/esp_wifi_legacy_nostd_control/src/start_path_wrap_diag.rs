...
use crate::timer_wrap_diag::snapshot_timer_wrap_counts;
...
fn capture_snapshot(fn_id: u8, args: [usize; 4], ret: usize, pre: bool) -> Snapshot {
    let timer_diag = snapshot_timer_wrap_counts();
    ...
    snap.fn_id = fn_id;
    snap.args = [args[0] as u32, args[1] as u32, args[2] as u32, args[3] as u32];
    snap.ret = ret as u32;
    if pre {
...
pub(crate) fn reset_start_path_wrap_diag() {
...
pub(crate) fn print_start_path_wrap_diag(label: &str) {
    let count = NEXT_SLOT.load(Ordering::Relaxed);
    println!(
        "legacy_nostd_wifi_control: start_path_wrap_diag label={} count={}",
        label, count
    );
    ...
    for idx in 0..limit {
        ...
        println!(
            "legacy_nostd_wifi_control: start_path_wrap_diag_entry label={} idx={} fn={} arg0=0x{:08x} arg1=0x{:08x} arg2=0x{:08x} arg3=0x{:08x} ret=0x{:08x} pre_timer_setfn={} post_timer_setfn={} pre_timer_arm={} post_timer_arm={} pre_sta_ptr=0x{:08x} post_sta_ptr=0x{:08x} pre_chm_ptr=0x{:08x} post_chm_ptr=0x{:08x} pre_home_chan=0x{:02x} post_home_chan=0x{:02x} pre_current_chan=0x{:02x} post_current_chan=0x{:02x} pre_op_chan=0x{:02x} post_op_chan=0x{:02x} pre_word114=0x{:08x} post_word114=0x{:08x}",
            label,
            idx,
            name,
            slot.args[0],
