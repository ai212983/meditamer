...
unsafe extern "C" {
    fn __real_scan_profile_check(a2: usize, a3: usize, a4: usize, a5: usize, a6: usize, a7: usize) -> usize;
    fn __real_scan_parse_beacon(a2: usize, a3: usize, a4: usize, a5: usize, a6: usize, a7: usize) -> usize;
}
...
unsafe fn wrap_call(
    ...
    let ptrs = [a2, a3, a4];
    let pre = [capture_fields(ptrs[0]), capture_fields(ptrs[1]), capture_fields(ptrs[2])];
    let ret = unsafe { real(a2, a3, a4, a5, a6, a7) };
    let post = [capture_fields(ptrs[0]), capture_fields(ptrs[1]), capture_fields(ptrs[2])];
...
pub(crate) fn reset_profile_wrap_diag() {
...
pub(crate) fn print_profile_wrap_diag(label: &str, phase: &str) {
    let count = NEXT_SLOT.load(Ordering::Relaxed).min(SLOT_COUNT);
    println!(
        "legacy_nostd_wifi_control: profile_wrap_diag label={} phase={} count={}",
        label, phase, count
    );
    for idx in 0..count {
        let snap = unsafe { SNAPSHOTS[idx] };
        println!(
            "legacy_nostd_wifi_control: profile_wrap_diag_entry label={} phase={} idx={} fn={} ret=0x{:08x} arg0=0x{:08x} arg1=0x{:08x} arg2=0x{:08x} arg3=0x{:08x}",
            label, phase, idx, fn_label(snap.fn_id), snap.ret, snap.args[0], snap.args[1], snap.args[2], snap.args[3]
        );
        for ptr_idx in 0..PTR_ARG_COUNT {
            let pre = snap.pre[ptr_idx];
            let post = snap.post[ptr_idx];
            println!(
                "legacy_nostd_wifi_control: profile_wrap_diag_fields label={} phase={} idx={} fn={} ptr_idx={} pre={:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x} post={:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}:{:02x}",
                label,
                phase,
                idx,
                fn_label(snap.fn_id),
