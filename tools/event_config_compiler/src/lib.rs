...
pub struct ImuToml {
    #[serde(default)]
    pub sampling: ImuSamplingToml,
}

#[derive(Debug, Deserialize)]
pub struct ImuSamplingToml {
...
impl Default for ImuSamplingToml {
    fn default() -> Self {
        Self {
            sensor_odr_hz: 416,
            idle_hz: 20,
            active_hz: 125,
            active_hold_ms: 1_000,
        }
    }
}
...
pub fn validate_config(events: &EventsToml) -> Result<(), ConfigCompilerError> {
    ...
    let sampling = &events.imu.sampling;

    if !matches!(
        sampling.sensor_odr_hz,
        13 | 26 | 52 | 104 | 208 | 416 | 833 | 1660
    ) {
        return Err(validation(
            "imu.sampling.sensor_odr_hz must be a supported LSM6DS3 ODR",
        ));
    }
    if sampling.idle_hz == 0 {
        return Err(validation("imu.sampling.idle_hz must be > 0"));
    }
    if sampling.active_hz < sampling.idle_hz {
        return Err(validation(
            "imu.sampling.active_hz must be >= imu.sampling.idle_hz",
        ));
    }
    if sampling.active_hz > 125 {
        return Err(validation(
            "imu.sampling.active_hz must be <= 125 for direct polling",
        ));
    }
    if sampling.active_hz > sampling.sensor_odr_hz {
        return Err(validation(
            "imu.sampling.active_hz must be <= imu.sampling.sensor_odr_hz",
        ));
    }
    ...
    if sampling.active_hold_ms < t.last_max_gap_ms {
        return Err(validation(
            "imu.sampling.active_hold_ms must be >= triple_tap.last_max_gap_ms",
        ));
    }
    if t.cooldown_ms == 0 {
    ...
    format!(
        "// @generated by build.rs from config/events.toml\n\
         pub const EVENT_ENGINE_CONFIG: EventEngineConfig = EventEngineConfig {{\n\
             imu_sampling: ImuSamplingConfig {{\n\
        ...
        sensor_odr_hz = sampling.sensor_odr_hz,
        idle_hz = sampling.idle_hz,
        active_hz = sampling.active_hz,
        active_hold_ms = sampling.active_hold_ms,
        enabled = t.enabled,
