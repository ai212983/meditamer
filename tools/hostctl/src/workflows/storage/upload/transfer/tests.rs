...
use crate::workflows::storage::upload::UploadRetryPolicy;
...
fn read_http_request(stream: &mut TcpStream) -> std::io::Result<(String, String, usize, usize)> {
...
fn direct_mode_repeated_transport_reset_falls_back_to_chunked_upload() {
    ...
    let server_thread = thread::spawn(move || {
        ...
        while Instant::now() < deadline {
            ...
            let Ok((method, target, content_length, prefetched)) = read_http_request(&mut stream)
            else {
                continue;
            };
            ...
            if method == "PUT" && target.starts_with("/upload?") {
                ...
                let _ = write_response(
                    ...
                    "direct should have fallen back",
                );
                continue;
            }
