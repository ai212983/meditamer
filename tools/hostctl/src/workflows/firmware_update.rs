...
pub fn run_firmware_update(logger: &mut Logger, opts: FirmwareUpdateOptions) -> Result<()> {
    let image = fs::read(&opts.image)
        .with_context(|| format!("read firmware image {}", opts.image.display()))?;
    let signing_key = read_signing_key(&opts.key)?;
    ...
    let log_path = opts.output.unwrap_or_else(|| {
        PathBuf::from(format!(
...
pub fn firmware_public_key_hex(path: &Path) -> Result<String> {
    Ok(hex(&read_signing_key(path)?.verifying_key().to_bytes()))
}
