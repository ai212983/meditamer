fn prepare_output_paths(explicit: Option<&Path>) -> Result<OutputPaths> {
...
fn normalize_output_root(explicit: Option<&Path>) -> (Option<PathBuf>, Option<String>) {
...
fn resolve_port(logger: &mut Logger, explicit: Option<&str>) -> Result<String> {
...
fn acquire_port_lock(port: &str) -> Result<PortLock> {
...
fn ensure_port_available(port: &str) -> Result<()> {
...
fn resolve_explicit_image(explicit: Option<&Path>, repo_dir: &Path) -> Result<PathBuf> {
...
fn build_firmware_command(profile: &str, repo_dir: &Path) -> Result<CommandSpec> {
    match profile {
        "debug" | "release" => {}
