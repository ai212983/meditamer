...
use crate::scenarios::conditions::eval_expression_value;
use crate::scenarios::conditions::extract_path_value;
use anyhow::anyhow;
use anyhow::Result;
...
pub(crate) fn should_run(common: &TaskDefinitionFields, context: &Value) -> Result<bool> {
    match &common.if_ {
...
pub(crate) fn resolve_runtime_value(value: &Value, context: &Value) -> Result<Value> {
    match value {
...
pub(crate) fn set_context_path(context: &mut Value, path: &str, value: Value) -> Result<()> {
    let root = context
        .as_object_mut()
...
pub(crate) fn error_matches_filter(
...
pub(crate) fn task_type_name(task: &TaskDefinition) -> &'static str {
    match task {
...
pub(crate) struct TaskIndex {
    pub(crate) tasks_by_name: HashMap<String, TaskDefinition>,
    pub(crate) ordered_names: Vec<String>,
    pub(crate) position: HashMap<String, usize>,
}

pub(crate) fn index_tasks(tasks: &WorkflowMap<String, TaskDefinition>) -> Result<TaskIndex> {
